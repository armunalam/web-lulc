...
from UnimatchV2_LULC.model.backbone.dinov2 import DINOv2
from UnimatchV2_LULC.model.util.blocks import FeatureFusionBlock, _make_scratch
...
class DPT(nn.Module):
    def __init__(
        ...
        self.head = DPTHead(nclass, self.backbone.embed_dim,
                            features, use_bn, out_channels=out_channels)
    ...
    def forward(self, x, comp_drop=False):
        ...
        patch_h, patch_w = x.shape[-2] // patch_size_temp, x.shape[-1] // patch_size_temp
        ...
        if comp_drop:
            ...
            features = (feature * dropout_mask.unsqueeze(1)
                        for feature in features)

            out = self.head(features, patch_h, patch_w)

            out = F.interpolate(out, (patch_h * patch_size_temp, patch_w *
                                patch_size_temp), mode='bilinear', align_corners=True)
